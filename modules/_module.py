...
class Module(ABC):
    ...
    def _print_options(self):
        print_formatted_text(
            HTML(f"<{ColorSelected().theme.accent}> Options (Field = Value)</{ColorSelected().theme.accent}>"))
        print(" -----------------------")
        flag = True
...
{key}  = <{ColorSelected().theme.confirm}>{option.value}</{ColorSelected().theme.confirm}> ({option.description})'''))
            ...
            print_formatted_text(HTML(
                f''' |_{key} = <{ColorSelected().theme.confirm}>{option.value} </{ColorSelected().theme.confirm}> ({option.description})'''))

    def _print_optional_option(self, key, option):
        if str(option.value) == "None":
            print_formatted_text(HTML(
                f" |_[OPTIONAL] {key} = <{ColorSelected().theme.confirm}>{option.value}</{ColorSelected().theme.confirm}> ({option.description})"))
        else:
            print_formatted_text(HTML(
                f" |_{key} = <{ColorSelected().theme.confirm}>{option.value}</{ColorSelected().theme.confirm}> ({option.description})"))
